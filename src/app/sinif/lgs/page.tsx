    ...
    useEffect(() => {
        async function fetchCourses() {
          try {
            ...
            const filteredCourses = allCourses.filter(course => course.category === 'LGS');
            setGradeCourses(filteredCourses);
          } catch (error) {
            console.error("Failed to fetch courses for LGS:", error);
          } finally {
            setLoading(false);
          }
        }
    ...
    <div className="bg-secondary/40 min-h-[calc(100vh-3.5rem)]">
        <div className="container py-8 md:py-12">
             <div className="mb-8">
                ...
                <h1 className="text-4xl font-bold text-primary">LGS Hazırlık Kaynakları</h1>
                <p className="text-lg text-muted-foreground mt-2">Sınava hazırlıkta en büyük yardımcınız.</p>
            </div>
           {loading ? (
